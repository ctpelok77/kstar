...
void PlanReconstructor::extract_plan(vector<Node>& seq,
	...
	for(;;) {
		...
		if(seq_index <= seq_size - 1 && seq[seq_index].sap->to == current_state.get_id()) {
			...
			//std::cout << "prepend seq " << seq[seq_index].sap->op->get_name() << std::endl;
			//std::cout << "from " << seq[seq_index].sap->get_from_state().get_state_tuple() << std::endl;
			//std::cout << "to " << seq[seq_index].sap->get_to_state().get_state_tuple() << std::endl;
            ...
            plan.push_back(op);
			//std::cout << "prepend tree " << op->get_name() << std::endl;
