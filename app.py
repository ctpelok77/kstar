...
plan_request = api.model(
    "PlanRequest",
    {
        "domain": fields.String(required=True, description="The domain.pddl as a string"),
        "problem": fields.String(required=True, description="The problem.pddl as a string"),
        "numplans": fields.Integer(required=True, description="The number of plans to return")
    }
)
...
plan_data = api.model(
    "PlanData",
    {
        ...
        "actions": fields.List(fields.String, required=True, description="The actions for the plan")
    }
)
...
plan_response = api.model(
    "PlanResponse",
    {
        "plans": fields.List(fields.Nested(plan_data), required=True, description="An array of plans")
    }
)
...
class KStarPlanner(Resource):
    ...
    def post(self, category, planner):
        ...
        search_args = "kstar(blind(),k={0},json_file_to_dump={1})".format(numplans, plan_file)
